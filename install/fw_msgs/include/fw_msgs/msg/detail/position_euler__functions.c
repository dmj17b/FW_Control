// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from fw_msgs:msg/PositionEuler.idl
// generated code does not contain a copyright notice
#include "fw_msgs/msg/detail/position_euler__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
fw_msgs__msg__PositionEuler__init(fw_msgs__msg__PositionEuler * msg)
{
  if (!msg) {
    return false;
  }
  // x_trans
  // y_trans
  // z_trans
  // roll
  // pitch
  // yaw
  return true;
}

void
fw_msgs__msg__PositionEuler__fini(fw_msgs__msg__PositionEuler * msg)
{
  if (!msg) {
    return;
  }
  // x_trans
  // y_trans
  // z_trans
  // roll
  // pitch
  // yaw
}

bool
fw_msgs__msg__PositionEuler__are_equal(const fw_msgs__msg__PositionEuler * lhs, const fw_msgs__msg__PositionEuler * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // x_trans
  if (lhs->x_trans != rhs->x_trans) {
    return false;
  }
  // y_trans
  if (lhs->y_trans != rhs->y_trans) {
    return false;
  }
  // z_trans
  if (lhs->z_trans != rhs->z_trans) {
    return false;
  }
  // roll
  if (lhs->roll != rhs->roll) {
    return false;
  }
  // pitch
  if (lhs->pitch != rhs->pitch) {
    return false;
  }
  // yaw
  if (lhs->yaw != rhs->yaw) {
    return false;
  }
  return true;
}

bool
fw_msgs__msg__PositionEuler__copy(
  const fw_msgs__msg__PositionEuler * input,
  fw_msgs__msg__PositionEuler * output)
{
  if (!input || !output) {
    return false;
  }
  // x_trans
  output->x_trans = input->x_trans;
  // y_trans
  output->y_trans = input->y_trans;
  // z_trans
  output->z_trans = input->z_trans;
  // roll
  output->roll = input->roll;
  // pitch
  output->pitch = input->pitch;
  // yaw
  output->yaw = input->yaw;
  return true;
}

fw_msgs__msg__PositionEuler *
fw_msgs__msg__PositionEuler__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  fw_msgs__msg__PositionEuler * msg = (fw_msgs__msg__PositionEuler *)allocator.allocate(sizeof(fw_msgs__msg__PositionEuler), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(fw_msgs__msg__PositionEuler));
  bool success = fw_msgs__msg__PositionEuler__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
fw_msgs__msg__PositionEuler__destroy(fw_msgs__msg__PositionEuler * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    fw_msgs__msg__PositionEuler__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
fw_msgs__msg__PositionEuler__Sequence__init(fw_msgs__msg__PositionEuler__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  fw_msgs__msg__PositionEuler * data = NULL;

  if (size) {
    data = (fw_msgs__msg__PositionEuler *)allocator.zero_allocate(size, sizeof(fw_msgs__msg__PositionEuler), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = fw_msgs__msg__PositionEuler__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        fw_msgs__msg__PositionEuler__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
fw_msgs__msg__PositionEuler__Sequence__fini(fw_msgs__msg__PositionEuler__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      fw_msgs__msg__PositionEuler__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

fw_msgs__msg__PositionEuler__Sequence *
fw_msgs__msg__PositionEuler__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  fw_msgs__msg__PositionEuler__Sequence * array = (fw_msgs__msg__PositionEuler__Sequence *)allocator.allocate(sizeof(fw_msgs__msg__PositionEuler__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = fw_msgs__msg__PositionEuler__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
fw_msgs__msg__PositionEuler__Sequence__destroy(fw_msgs__msg__PositionEuler__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    fw_msgs__msg__PositionEuler__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
fw_msgs__msg__PositionEuler__Sequence__are_equal(const fw_msgs__msg__PositionEuler__Sequence * lhs, const fw_msgs__msg__PositionEuler__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!fw_msgs__msg__PositionEuler__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
fw_msgs__msg__PositionEuler__Sequence__copy(
  const fw_msgs__msg__PositionEuler__Sequence * input,
  fw_msgs__msg__PositionEuler__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(fw_msgs__msg__PositionEuler);
    fw_msgs__msg__PositionEuler * data =
      (fw_msgs__msg__PositionEuler *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!fw_msgs__msg__PositionEuler__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          fw_msgs__msg__PositionEuler__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!fw_msgs__msg__PositionEuler__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
