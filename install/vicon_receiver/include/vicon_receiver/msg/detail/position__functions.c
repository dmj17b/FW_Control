  ...
  vicon_receiver__msg__Position__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    vicon_receiver__msg__Position * data =
      (vicon_receiver__msg__Position *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!vicon_receiver__msg__Position__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          vicon_receiver__msg__Position__fini(&data[i]);
        }
        free(data);
        return false;
      }
